...
namespace ERP.UI.ViewModel
{
    ...
    internal class HauptfensterViewModel : BaseViewModel
    {
        ...
        private async Task StartDashboard()
        {
            try
            {
                await Task.Delay(200); // Kurze Verzögerung, um sicherzustellen, dass das Hauptfenster geladen ist
                var dashboard = new Dashboard();

                var hauptfenster = Application.Current.Windows
                    .OfType<Hauptfenster>()
                    .FirstOrDefault(w => w is Views.Hauptfenster);
                if (hauptfenster != null)
                {
                    var contentHost = hauptfenster.FindName("ContentHost") as ContentControl;
                ...
                {
                    Application.Current.MainWindow.DataContext = dashboard;
                }
            }
            catch(Exception ex)
            {
               /* MessageBox.Show($"Fehler beim Starten des Dashboards: {ex.Message}", 
